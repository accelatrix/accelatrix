...
var Accelatrix,__assign=this&&this.__assign||function(){return(__assign=Object.assign||function(t){for(var s,i=1,n=arguments.length;i<n;i++)for(var p in s=arguments[i])Object.prototype.hasOwnProperty.call(s,p)&&(t[p]=s[p]);return t}).apply(this,arguments)};!function(Accelatrix_1){!function(Serialization){var SerializationSystem;Serialization.FromJSON=function(json,type){return SerializationSystem.ParseJSON(json,type)},Serialization.ToJSON=function(obj){return SerializationSystem.Stringify(obj)},function(SerializationSystem){function GetTypeFromHint(typeName){var candidates=Accelatrix.Type.FromName(typeName);if(null==candidates||0==candidates.length)throw new Accelatrix.TypeNotFoundException(typeName);if(1!=candidates.length)throw new Accelatrix.ArgumentException(typeName);return candidates[0]}function GetPropertyType(type,propertyName){return null==type||String.IsNullOrWhiteSpace(propertyName)||0==(type=type.GetProperties().map(function(z){return{Name:z.Name,Type:z.Type}}).concat(type.GetFields().map(function(z){return{Name:z.Name,Type:z.Type}})).filter(function(z){return z.Name==propertyName}).map(function(z){return z.Type}).filter(function(z){return null!=z})).length?null:type[0]}function ObjectToClass(obj,type){try{var newValue=type.CreateNewInstance(obj);target=newValue,null!=(from=obj)&&null!=target&&Object.keys(from).filter(function(z){return null!=from[z]}).filter(function(z){return null==target[z]}).forEach(function(z){try{target[z]=from[z]}catch(ex){Object.defineProperty(target,z,{get:function(){return from[z]},enumerable:!0})}}),obj=newValue}catch(ex){}var target,from;return obj}SerializationSystem.ParseJSON=function(json,type){if(String.IsNullOrWhiteSpace(json))throw new Accelatrix.ArgumentNullException("json");var targetType;if(null!=type&&"String"==type.GetType().Name){var myType=Accelatrix.Type.FromName(type);if(null==myType||0==myType.length)throw new Accelatrix.TypeNotFoundException(type);if(1!=myType.length)throw new Accelatrix.AmbiguousTypeException(type,null,myType);targetType=myType[0]}else targetType=null!=type&&type.GetType().IsFunction?Accelatrix.Type.FromConstructor(type):type;var rootObj=null,myType=JSON.parse(json,function(key,value){if(null==rootObj&&(rootObj=this),null!=value&&value instanceof Object){var proposedType=void 0;if(String.IsNullOrWhiteSpace(value.$type))if(this==rootObj)proposedType=GetPropertyType(targetType,key);else try{proposedType=GetPropertyType(GetTypeFromHint(this.$type),key)}catch(ex2){}else try{proposedType=GetTypeFromHint(value.$type)}catch(ex){if(this==rootObj)proposedType=GetPropertyType(targetType,key);else try{proposedType=GetPropertyType(GetTypeFromHint(this.$type),key)}catch(ex2){}}null==proposedType||proposedType.IsPrimitive||(value=ObjectToClass(value,proposedType))}return value});if(Object.UnboxDates(myType),null==targetType){if(null==myType||String.IsNullOrWhiteSpace(myType.$type))throw new Accelatrix.ArgumentNullException("A target type was not specified, nor the deserialized object contains a $type hint.","type");targetType=GetTypeFromHint(myType.$type)}return ObjectToClass(myType,targetType)},SerializationSystem.Stringify=function(obj){return JSON.stringify(obj,function(key,value){if(value instanceof Object&&String.IsNullOrWhiteSpace(value.$type)&&!value.GetType().IsArray)return __assign({$type:value.GetType().GetFullName()},value);return value})}}(SerializationSystem=SerializationSystem||{})}(Accelatrix_1.Serialization||(Accelatrix_1.Serialization={}))}(Accelatrix=Accelatrix||{});